            ...
            <div class="user-row">
                ...
                <div class="header">
                    <ul>
                        ...
                        <li><a href="/src/logout.php" class="btn btn-sm btn-black">LOGOUT</a></li>
                    </ul>
                </div>
                <?php
                /** Model Area */
                ...
                $sql = "SELECT o.id,o.status,o.order_id,oi.id as oi_id,oi.name,oi.size,oi.product,oi.color,oi.design,oi.in_stock,oi.notes,oi.status,o.created_at FROM orders as o LEFT JOIN order_items as oi ON o.id = oi.order_id where oi.status='pending'";
                $result = $conn->query($sql);
...
<script>
    ...
    $(document).ready(function() {
        ...
        var table=$('#myTable').DataTable(
             ...
             { "order": 
               [[<?php if(!empty($_SESSION['index']['column'])) { echo $_SESSION['index']['column']; } else { echo 0; } ?>, '<?php if(!empty($_SESSION['index']['direction'])) { echo $_SESSION['index']['direction']; } else { echo "asc"; } ?>']]
            ,paging: false
            }
