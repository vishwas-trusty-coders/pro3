    ...
    <div class="container">
        ...
        <div id="all-user-data">
            <div class="user-row">
                ...
                //$sql = "SELECT * FROM orders where status='production'";

                $sql = "SELECT o.id,o.status,o.order_id,oi.id as oi_id,oi.name,oi.size,oi.product,oi.color,oi.design,oi.in_stock,oi.notes,oi.status,o.created_at FROM orders as o LEFT JOIN order_items as oi ON o.id = oi.order_id where oi.status='production'";
                $result = $conn->query($sql);
...
<script>
    ...
    $(document).ready(function() {
        var table=$('#myTable').DataTable(
            { "order": 
               [[<?php if(!empty($_SESSION['production']['column'])) { echo $_SESSION['production']['column']; } else { echo 0; } ?>, '<?php if(!empty($_SESSION['production']['direction'])) { echo $_SESSION['production']['direction']; } else { echo "asc"; } ?>']]
               ,paging: false
            }
    );
