    ...
    <div class="container">
        ...
        <div id="all-user-data">
            <div class="user-row">
                ...
                $sql = "SELECT o.id,o.status,o.order_id,oi.id as oi_id,oi.name,oi.size,oi.product,oi.color,oi.design,oi.in_stock,oi.notes,oi.progress,o.created_at FROM orders as o LEFT JOIN order_items as oi ON o.id = oi.order_id where o.status='production'";
                $result = $conn->query($sql);
                    ...
                    <table class="user-data-table" style="width:100% !important;" id="myTable">
                        ...
                        <tbody>
                            <?php while ($row = $result->fetch_assoc()) {
                                ...
                                . "<td class='td-name'>"; ?>
                                <input type='checkbox' class='checkbox-in-stock' data-id=<?php echo $row['oi_id']; ?> <?php if (!empty($row['in_stock'])) { echo 'checked'; } ?>>
                                <?php 
                                echo "</td>"
...
$(document).on('change', '.checkbox1', function() {
    ...
    if (checked) {
        //console.log(`Checkbox with ID ${id} is checked`);
         var form_data = new FormData();
            form_data.append('id', id);
            form_data.append('status', 'invoice');
            $.ajax({
                url: 'https://production.3pro.ca/src/orders/update.php',
                type: 'POST',
                data: form_data,
